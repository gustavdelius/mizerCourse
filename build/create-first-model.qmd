...
```{r create-first-model-8, warning=FALSE}
...
```{r create-first-model-10, warning=FALSE}
...
```{r create-first-model-11, warning=FALSE}
...
```{r create-first-model-12}
plotlySpectra(cel_model4, biomass = TRUE, per_log_size = TRUE)
```

There is a similar function `matchNumbers()` that you would use in case total numbers of individuals are known instead of total biomasses.
...
```{r create-first-model-17, warning=FALSE, message=FALSE}
cel_model7 <- tuneSteadyState(cel_model6) 
plotlySpectra(cel_model7, biomass = TRUE, per_log_size = TRUE, total = TRUE)
```

You see a pattern emerging. Whenever we have made a change to the system we have to run the dynamics with the `tuneSteadyState()` function to get to the new steady state.

## Step 8: Rinse and repeat

Running to steady state has again messed up our biomasses:

```{r create-first-model-18, warning=FALSE}
...
```{r create-first-model-20, message=FALSE, warning=FALSE}
...
(There are possible variations of this. You could leave out the `calibrateBiomass()` steps. You could insert an additional `tuneSteadyState()` step between `matchBiomasses()` and `matchGrowth()`. Some variants may converge faster than others, but it really makes no practical difference because this is so fast anyway.
)
It turns out that in this example iterating three times more was enough. Even in the steady state the biomasses are now spot on:

```{r create-first-model-21, warning=FALSE}
plotBiomassVsSpecies(cel_model8)
```

And the growth rates too are matched much more precisely than really necessary:

```{r create-first-model-22}
...
2.  Find a coexistence steady state with `tuneSteadyState()`.
