...
For each species we have numbers of individuals observed in 1cm wide length bins (the `dl` column indicates the width of each bin). If you get an error message saying that the `catch.csv` file can not be found, run the `download.file()` command above to fetch it.
...
Now that we have changed fishing selectivity we need to find a new steady state. It is important to run `tuneSteadyState()` frequently, while making small changes to the model parameters. If we accumulate too many changes, finding a new steady state might be harder.

```{r landings-11, message=FALSE, warning=FALSE}
