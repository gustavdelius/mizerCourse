...
```{r observed-size-spectra-3, eval=FALSE}
...
where the coefficient $a$ and the exponent $b$ are species-specific parameters (we'll discuss how to find such species parameters in a [later tutorial](../build/collect-parameters.qmd)).
...
```{r observed-size-spectra-6, message=FALSE}
p <- ggplot(size_data) +
...
```{r observed-size-spectra-7, message=FALSE}
p + scale_y_log10()
```
...
```{r observed-size-spectra-9, message=FALSE}
p2 <- ggplot(size_data) +
    geom_histogram(aes(weight), fill = "blue", colour = "black",
                   breaks = bin_breaks) +
    labs(x = "Weight [g]",
...
```{r observed-size-spectra-10}
log_breaks_22 <- seq(from = 0, to = 11, by = 1/2)
log_breaks_22
```

```{r observed-size-spectra-11}
bin_breaks_22 <- 2 ^ log_breaks_22
bin_breaks_22
```

```{r observed-size-spectra-12}
...
```{r observed-size-spectra-15, eval=FALSE}
...
```{r observed-size-spectra-18, warning=FALSE}
binned_numbers <- binned_numbers |>
...
```{r observed-size-spectra-19, warning=FALSE}
p_number_density + 
    scale_x_log10() + 
    scale_y_log10()
```
...
```{r observed-size-spectra-27, message=FALSE}
...
```{r observed-size-spectra-32, message=FALSE}
...
```{r observed-size-spectra-34, warning=FALSE}
p <- ggplot(size_data) +
