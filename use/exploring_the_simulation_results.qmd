---
title: "Explore the simulation results"
---

```{r exploring_the_simulation_results-1, include=FALSE}
library(mizer)
...
In the sections on
[running a simulation](running_a_simulation.qmd) we saw how to set up a model
...
do?  How can we explore the results of the simulation? In this tutorial we
...
[previous section on running a simulation](running_a_simulation.qmd):

```{r exploring_the_simulation_results-2, label = exploration_simulation}
...
have just run, the time dimension of `n` has `r dimN` rows
...
```{r exploring_the_simulation_results-4, label=show_dim_n}
dim(N(sim))
```
...
```{r exploring_the_simulation_results-5, results='hide'}
N(sim)[, "Cod", ]
```
...
```{r exploring_the_simulation_results-6, results='hide'}
finalN(sim)
```
...
```{r exploring_the_simulation_results-7, results='hide'}
NResource(sim)
```
...
```{r exploring_the_simulation_results-8, results='hide'}
finalNResource(sim)
```
...
[the summary functions help page.](https://sizespectrum.org/mizer/reference/summary_functions.html)
...
[the species parameters section](https://sizespectrum.org/mizer/articles/multispecies_model.html#sec:species_parameters_dataframe)) 
to include
...
```{r exploring_the_simulation_results-9, label = getSSB_demo}
ssb <- getSSB(sim)
dim(ssb)
head(ssb)
```
...
```{r exploring_the_simulation_results-10, label = getBiomass_demo}
...
available is given in [the indicator functions help page.](https://sizespectrum.org/mizer/reference/indicator_functions.html).
...
[the section on summary functions for MizerSim objects.](exploring_the_simulation_results.qmd#sec:summaries).
It is also
...
```{r exploring_the_simulation_results-11, label=getCommunitySlope_demo}
...
```{r exploring_the_simulation_results-12, label = getCommunitySlope_with_args}
...
inputs (see [the plots help page.](https://sizespectrum.org/mizer/reference/plotting_functions.html)). As
...
```{r exploring_the_simulation_results-13, label=plotBiomass, fig.keep='last'}
plotBiomass(sim)
```
...
```{r exploring_the_simulation_results-14, label=plotSpectra_example, fig.keep='last'}
...
```{r exploring_the_simulation_results-15, label=demo_summary_plot}
plot(sim)
```

The next section describes how to use what we have learned to 
[model the North Sea.](a_multispecies_model_of_the_north_sea.qmd) 
