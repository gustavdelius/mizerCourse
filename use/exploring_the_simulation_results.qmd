...
have just run, the time dimension of the returned array has `r dimN` rows
...
`dim()` function and looking at the dimensions of the array returned by `N(sim)`:
...
data.frames using `as.data.frame()`.
...
```{r plotSpectra_example, fig.keep='last'}
...
There is much more to be said about analysing and plotting simulation results. Take a look at the [Guide: Analysing and plotting results](https://sizespectrum.org/mizer/articles/guide-analyse-and-plot.html).
