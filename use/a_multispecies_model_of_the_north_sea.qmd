---
title: "A simulation of the North Sea"
---

```{r a_multispecies_model_of_the_north_sea-1, include=FALSE}
library(mizer)
species_str <- paste(NS_species_params[,"species"], collapse=", ")
```
...
`k_vb`, `R_max` and `w_inf`.

```{r a_multispecies_model_of_the_north_sea-2}
NS_species_params
```
...
calibration process which is not covered here.
...
```{r a_multispecies_model_of_the_north_sea-3}
...
[the section on fishing gears and selectivity](https://sizespectrum.org/mizer/articles/multispecies_model.html#sec:fishing_gear), 
...
```{r a_multispecies_model_of_the_north_sea-4, label=adding_sel_params}
gear_params <- 
...
```{r a_multispecies_model_of_the_north_sea-5, label=show_load_f_history}
...
```{r a_multispecies_model_of_the_north_sea-6, label=head_f_history}
head(f_history)
```
...
```{r a_multispecies_model_of_the_north_sea-7, label=set_catchability}
...
spectrum (see [see the section on resource density](https://sizespectrum.org/mizer/articles/model_description.html#resource-density)). 
This was estimated along with the values `R_max` as part of the calibration
process. 

We will use the
interaction matrix `NS_interaction` that is shipped as an example with the mizer package, see also (https://sizespectrum.org/mizer/articles/multispecies_model.html#setting-the-interaction-matrix).

We now have all the information we need to create the `MizerParams` object:

```{r a_multispecies_model_of_the_north_sea-8, label=make_ns_params, results='hide'}
...
```{r a_multispecies_model_of_the_north_sea-9, label=rescale_effort}
...
```{r a_multispecies_model_of_the_north_sea-10}
...
```{r a_multispecies_model_of_the_north_sea-11, label=project_ns_model}
...
```{r a_multispecies_model_of_the_north_sea-12, label = plot_ns_biomass}
plotBiomass(sim)
```
...
```{r a_multispecies_model_of_the_north_sea-13, label=unexploited_na}
...
[the section about indicator functions](exploring_the_simulation_results.qmd#functions-for-calculating-indicators).
...
species larger than 40 cm. Because we are interested only in the equilibrium
unexploited values, we apply the indicator functions to `finalParams(sim0)`, the
`MizerParams` object holding the state at the final time step of the simulation.
Applied to a `MizerParams` object rather than a `MizerSim`, these functions
return the single value for that state rather than a whole time series.

```{r a_multispecies_model_of_the_north_sea-14, label=ns_comm_ref_point}
...
```{r a_multispecies_model_of_the_north_sea-15, label=ns_comm_indicators}
...
```{r a_multispecies_model_of_the_north_sea-16, label=plot_ns_indicators, fig.cap="Historical (solid) and unexploited (dashed) and reference (dotted) community indicators for the North Sea multispecies model."}
library(ggplot2)
years <- 1967:2010
# Simulated data
...
According to our simulations, the LFI in the North Sea stayed below the
reference level for most of the historical period, only rising above it in
the last few years of the simulation.
...
```{r a_multispecies_model_of_the_north_sea-17, label = ns_scenario1_relative_effort}
...
```{r a_multispecies_model_of_the_north_sea-18, label = ns_scenario2_relative_effort}
...
```{r a_multispecies_model_of_the_north_sea-19, label = project_ns_future_scenarios}
...
```{r a_multispecies_model_of_the_north_sea-20, label=ns_biodiv_ref_point}
ssb0 <- getSSB(params0)
```
...
```{r a_multispecies_model_of_the_north_sea-21, label=projected_ssb_ns, fig.cap="Historical and projected SSB under two fishing scenarios. Status quo (red), Fmsy (yellow). Unexploited (blue) and reference levels (purple) are also shown."}
years <- 1967:2050
