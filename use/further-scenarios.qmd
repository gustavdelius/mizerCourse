...
```{r further-scenarios-10, ,message=FALSE}
...
```{r further-scenarios-11, , eval = T, echo = T, warning=F}
...
```{r further-scenarios-14, , code_folding=TRUE}
...
```{r further-scenarios-15, , code_folding=TRUE}
...
```{r further-scenarios-16, ,code_folding=TRUE}
...
The `finalParams()` function has returned the model with its initial spectra and initial effort set to the final values from `simf3`, i.e., to the values from 2020. The effort was

```{r further-scenarios-20}
initial_effort(params)
```
...
```{r further-scenarios-23, ,code_folding=T}
...
```{r further-scenarios-29, , warning=F}
# plot change in biomass under each scenario relative to unfished values
# get the unfished biomasses
