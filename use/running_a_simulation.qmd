...
```{r running_a_simulation-1, include=FALSE}
library(mizer)
```
...
effort pattern. In this tutorial we take a detailed look at how the
...
The `MizerParams` class was explored in the previous tutorial. In this tutorial we
...
seen in 
[the section on projection examples.](running_a_simulation.qmd#sec:projection_examples)
...
[the section on exploring the simulation results.](exploring_the_simulation_results.qmd#).
...
We will use a MizerParams object with four gears.
...
```{r running_a_simulation-3, label=siple_projection_single_effort}
...
```{r running_a_simulation-4, label=plot_basic_ms_sim}
plot(sim)
```
...
```{r running_a_simulation-5, label=show_effort}
head(getEffort(sim))
```

`getEffort()` returns the effort by time and gear. In this example, we
...
```{r running_a_simulation-6, label=simple_proj_tsave05}
...
```{r running_a_simulation-7, label=set_effort_vector}
...
```{r running_a_simulation-8, label=project_effort_vector}
...
```{r running_a_simulation-9, label=plotFMort, fig.keep='last'}
plotFMort(sim)
```
...
```{r running_a_simulation-10, label=set_empty_effort_array}
...
```{r running_a_simulation-11, label=fill_effort_array}
...
```{r running_a_simulation-12, label=show_effort_array}
head(effort_array)
```
...
```{r running_a_simulation-13, label=project_effort_array}
...
it may be easier to prepare it in advance as a .csv file and read it in. We
give an example of this in 
[the section on the North Sea model.](a_multispecies_model_of_the_north_sea.qmd)
...
[explore the simulation results.](exploring_the_simulation_results.qmd)
