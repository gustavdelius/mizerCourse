...
1.  [**Run a simulation**](running_a_simulation.qmd)\
    We take a detailed look at how the
    `project()` function works and the different ways in which effort and time can
    be set up.

2.  [**Explore the simulation results**](exploring_the_simulation_results.qmd)\
    How can we explore the results of the simulation? In this tutorial we
    introduce a range of summaries, plots and indicators that can be easily produced
    using functions included in mizer.

3.  [**A simulation of the North Sea**](a_multispecies_model_of_the_north_sea.qmd)\
    In this section we try to pull everything together with an extended example of a
    multispecies model for the North Sea
...
{{< video https://youtu.be/Z8FW7t4xrtU >}}
