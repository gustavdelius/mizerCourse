...
There are three ways of setting the fishing effort. Examples of all three can be seen in [the section on projection examples.](../use/running_a_simulation.llms.md#sec:projection_examples)
...
In this section we’ll look at how to run simulations with the `project()` function. The examples will focus on how fishing effort can be specified in different ways. The results of the simulations will not be explored in detail. We will leave that for [the section on exploring the simulation results.](../use/exploring_the_simulation_results.llms.md#).
...
![](running_a_simulation_files/figure-html/plot_basic_ms_sim-1.png)
...
The effort through time can be inspected with `getEffort()` (we use the [`head()`](https://rdrr.io/r/utils/head.html) function to just show the first few lines).
...
`getEffort()` returns the effort by time and gear. In this example, we specified the `effort` argument as a single numeric of value 1. As you can see this results in the same effort being used for all gears for all time steps.
...
We then call `project()` with this effort and inspect the resulting `effort` matrix (again we use the [`head()`](https://rdrr.io/r/utils/head.html) function to just show the first few lines):
...
![](running_a_simulation_files/figure-html/plotFMort-1.png)
...
Then we fill it up, one gear at a time, making heavy use of the [`seq()`](https://rdrr.io/r/base/seq.html) function to create a sequence:
...
As you can see, it can be quite fiddly to set up a complicated effort array so it may be easier to prepare it in advance as a .csv file and read it in. We give an example of this in [the section on the North Sea model.](../use/a_multispecies_model_of_the_north_sea.llms.md)
...
Now that we know how to run a simulation, the we are ready to learn how to [explore the simulation results.](../use/exploring_the_simulation_results.llms.md)
