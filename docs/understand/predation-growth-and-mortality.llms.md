...
growth_rates_plot <- plot2(getEGrowth(params), getEGrowth(params_starved),
                           name1 = "Original", name2 = "Less prey", log = "xy")
growth_rates_plot
```
...
> **NOTE:**
>
> Mizer likes to work with arrays instead of data frames. I hope you noticed the magic how R can do calculations with all entries of an array in one go.
>
> ggplot2 on the other hand likes to work with data frames instead of arrays. The function [`melt()`](https://rdrr.io/pkg/reshape2/man/melt.html) allows us to convert the mizer arrays into data frames suitable for [`ggplot()`](https://ggplot2.tidyverse.org/reference/ggplot.html). Most mizer arrays that are returned by mizer functions have a [`plot()`](https://rdrr.io/r/graphics/plot.default.html) method, so you don’t need to use [`melt()`](https://rdrr.io/pkg/reshape2/man/melt.html) for those. But there is no mizer function for plotting the predation kernel, so we had to use [`melt()`](https://rdrr.io/pkg/reshape2/man/melt.html) before passing it to [`ggplot()`](https://ggplot2.tidyverse.org/reference/ggplot.html).
