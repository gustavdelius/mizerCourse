...
``` downlit
plotlySpectra(cel_model, biomass = TRUE, per_log_size = TRUE, total = TRUE)
```
...
``` downlit
cel_model <- cel_model |> matchGrowth() |> tuneSteadyState()
```
...
``` downlit
cel_model <- cel_model |> matchBiomasses() |> tuneSteadyState()
```
